...
{
	mMainWindowTitle = "Ch12 The Compute Shader";
...
TestApp::~TestApp()
{
	...
	//SafeRelease((*mScreenQuadSRV.GetAddressOf()));
...
bool TestApp::Init()
{
	if (D3DApp::Init())
	{
		std::wstring base = L"C:/Users/D3PO/source/repos/3D Game Programming with DirectX 11/Chapter 12 The Compute Shader/";
		...
		{
			std::wstring path = base + L"HorBlurCS.hlsl";
		...
		{
			std::wstring path = base + L"VerBlurCS.hlsl";
		...
		// vertex shader screen quad
		{
			...
			std::wstring path = base + L"ScreenQuadVS.hlsl";
		...
		// CS water update
		{
			std::wstring path = base + L"WaterUpdateCS.hlsl";
...
void TestApp::OnResize(GLFWwindow* window, int width, int height)
{
	...
	{
		SafeRelease(mScreenQuadSRV);
		//SafeRelease((*mScreenQuad.mSRV.GetAddressOf()));
		SafeRelease(mScreenQuadRTV);
		SafeRelease(mScreenQuadUAV);
		...
		//HR(mDevice->CreateShaderResourceView(texture, nullptr, mScreenQuad.mSRV.GetAddressOf()));
...
void TestApp::DrawScene()
{
	...
	mContext->OMSetRenderTargets(1, &mScreenQuadRTV, mDepthStencilView);

	mContext->ClearRenderTargetView(mScreenQuadRTV, Colors::Silver);

...
TestApp::TestApp()
	: D3DApp()
{
	mMainWindowTitle = "Ch08 Texturing";
...
bool TestApp::Init()
{
	if (D3DApp::Init())
	{
		...
		// shader resource view
		{
			...
			HR(CreateDDSTextureFromFile(mDevice, L"grass.dds", &resource, &mGrid.mSRV));
			...
			//mContext->PSSetShaderResources(0, 1, &mGrid.mSRV);

			HR(CreateDDSTextureFromFile(mDevice, L"water2.dds", nullptr, &mWaves.mSRV));
			// mContext->PSSetShaderResources(1, 1, &mSRV1);
		} // shader resource view
